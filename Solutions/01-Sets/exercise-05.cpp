#include "exercise-05.h"

#include <algorithm>
#include <random>

ex5::Set::Set(std::set<int> set)
	: set(std::move(set))
{
}

ex5::Set::Set(const std::vector<int>& vec)
	: set(vec.begin(), vec.end())
{
}

ex5::Set& ex5::Set::operator-=(const Set& B)
{
	if (this == &B)
	{
		set = {};
		return *this;
	}

	std::for_each(B.set.begin(), B.set.end(), [&](int i)
		{
			const auto it = std::find(set.begin(), set.end(), i);
			if (it != set.end())
				set.erase(it);
		}
	);

	return *this;
}

ex5::Set ex5::Set::operator-(const Set& B) const
{
	return Set(*this) -= B;
}

ex5::Set& ex5::Set::operator|=(const Set& B)
{
	if (this == &B)
	{
		return *this;
	}

	std::for_each(B.set.begin(), B.set.end(), [&](int i)
		{
			set.emplace(i);
		}
	);

	return *this;
}

ex5::Set ex5::Set::operator|(const Set& B) const
{
	return Set(*this) |= B;
}

ex5::Set& ex5::Set::operator&=(const Set& B)
{
	if (this == &B)
	{
		return *this;
	}

	std::set<int> intersection;
	std::for_each(set.begin(), set.end(), [&](int i)
		{
			if (std::find(B.set.begin(), B.set.end(), i) != B.set.end())
				intersection.emplace(i);
		}
	);

	set = intersection;

	return *this;
}

ex5::Set ex5::Set::operator&(const Set& B) const
{
	return Set(*this) &= B;
}

std::vector<ex5::Set> ex5::Set::power() const
{
	std::vector<Set> subsets{ Set(std::set<int>{}) };

	for (auto it = set.begin(); it != set.end(); ++it)
	{
		std::set<int> subset;
		subset.insert(*it);

		auto vec = power_next(it, set.end(), subset);

		subsets.emplace_back(subset);

		subsets.reserve(subsets.size() + vec.size());
		std::move(std::begin(vec), std::end(vec), std::back_inserter(subsets));
	}

	return subsets;
}

bool ex5::Set::contains(const Set& B) const
{
	if (set.size() < B.set.size())
	{
		return false;
	}

	return std::find_if(B.set.begin(), B.set.end(), [this](int element)
		{
			return set.find(element) == set.end();
		}
	) == B.set.end();
}

int ex5::Set::random() const
{
	static std::random_device rd;
	static std::mt19937 gen(rd());
	auto it = set.begin();

	std::uniform_int_distribution<> dis(0, std::distance(it, set.end()) - 1);
	std::advance(it, dis(gen));
	
	return *it;
}

void ex5::Set::printShuffled() const
{
	static std::random_device rd;
	static std::mt19937 gen(rd());
	
	std::vector<int> copy(set.begin(), set.end());
	std::shuffle(copy.begin(), copy.end(), gen);

	std::cout << "{ ";
	std::copy(copy.begin(), copy.end(), std::ostream_iterator<int>(std::cout, ", "));
	std::cout << "}" << std::endl;
}

std::vector<std::set<int>> ex5::Set::power_next(
	std::set<int>::iterator current,
	std::set<int>::iterator end,
	const std::set<int>& subset) const
{
	std::vector<std::set<int>> subsets;

	for (auto it = ++current; it != end; ++it)
	{
		std::set<int> expansion = subset;
		expansion.insert(*it);

		auto vec = power_next(it, end, expansion);

		subsets.push_back(std::move(expansion));

		subsets.reserve(subsets.size() + vec.size());
		std::move(std::begin(vec), std::end(vec), std::back_inserter(subsets));
	}

	return subsets;
}

std::ostream& ex5::operator<<(std::ostream& out, const Set& set)
{
	out << "{ ";
	std::for_each(set.set.begin(), set.set.end(), [&out](int i) { out << i << ", "; });
	out << "}";
	return out;
}

int ex5::factorial(int n)
{
	if (n < 0)
	{
		return -1;
	}
	if (n == 0)
	{
		return 1;
	}
	return n * factorial(n - 1);
}
